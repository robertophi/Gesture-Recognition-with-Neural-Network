...
while (1):
    ...
    mask2 = cv2.inRange(hsv, np.array([2, 50, 50]), np.array([h+10, 255, 255]))
    mask2[:,320:] = 0
...
try:
    ...
    array = np.zeros((1, len(biggestContour), 2))
    for i in range(len(biggestContour)):
        if not (biggestContour[i][0][0] < centroid_x - w):
            array[0][i] = biggestContour[i]
    biggestContour = array.astype(int)
except:
    print("error array")
    pass
...
try:
    ...
    for i in range(defects.shape[0]):
        s, e, f, d = defects[i, 0]
        if (d > 200):
            start = tuple(biggestContour[0][s])
            end = tuple(biggestContour[0][e])
            far = tuple(biggestContour[0][f])
